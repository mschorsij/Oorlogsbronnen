...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="europeana:record">
		<xsl:if
			test="contains(dc:subject, 'geschiedenis 1940-1945')
			or contains(dc:subject, 'Geschiedenis 1940-1945')
			or contains(dc:subject, '1940')
			or contains(dc:subject, '1945')">
	...
	<xsl:template name="dc_record">
		...
		<spinque:attribute
			...
			value="Beeldbank Zeeuwse Bibliotheek"
			type="string"/>
		<spinque:attribute
			subject="{$subject}"
			attribute="schema:disambiguatingDescription"
			value="In Oorlogsbronnen in set beeldbank_zeeuwsebibliotheek"
			type="string"/>
		<spinque:attribute
			subject="{$subject}"
			attribute="dc:source"
		...
		<spinque:relation
			...
			object="niod:Organizations/397"/>
		<spinque:attribute
			subject="{$subject}"
			attribute="dc:publisher"
			value="ZB Planbureau en Bibliotheek van Zeeland"
			type="string"/>
		<!-- end -->
	...
	<xsl:template match="dcterms:spatial">
		...
		<xsl:variable name="spatial"
			select="su:uri('http://data.oorlogsbronnen.nl/beeldbank_zeeuwsebibliotheek/spatial/', .)"/>
		...
		<spinque:relation subject="{$spatial}" object="schema:Place"
			predicate="rdf:type"/>
		<spinque:attribute type="string" value="{.}" attribute="schema:name"
			subject="{$spatial}"/>
		<spinque:attribute
			type="string"
			value="{.}"
			...
			<xsl:when test="string-length(.) = 10">
				<xsl:choose>
					<xsl:when test="contains(., '-00-00')">
						<!--<dcterms:created>1960-00-00</dcterms:created>-->
						<spinque:attribute subject="{$subject}"
							attribute="dc:date" type="date"
							value="{su:parseDate(substring-before(., '-00-00'),'yyyy')}"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="contains(., '-00')">
								<spinque:attribute subject="{$subject}"
									attribute="dc:date" type="date"
									value="{su:parseDate(substring-before(., '-00'),'yyyy-MM')}"/>
							</xsl:when>
							<xsl:otherwise>
								<spinque:attribute subject="{$subject}"
									attribute="dc:date" type="date"
									value="{su:parseDate(.,'yyyy-MM-dd')}"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
